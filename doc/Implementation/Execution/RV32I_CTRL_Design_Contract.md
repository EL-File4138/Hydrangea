...
CTRL shall compute the complete architectural next-PC value for every operation it accepts. The core shall use this result directly for a control-transfer instruction and shall not independently apply a fall-through increment.
...
Changes to numerical operation encodings or port names belong to RTL and tests. This contract requires revision only if operand ownership, PC identity, result meaning, or core/CTRL responsibility changes.
