...
module sync_ram #(
  // No larger than 30 for a 32 bit address space (byte-addressed)
    ...
    input rv32_inst_pkg::mem_width_e write_type_i,
  ...
  logic [31:0] mem_cell[2**AddrWidth];
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      unique case (write_type_i)
        rv32_inst_pkg::MEM_WIDTH_BYTE: mem_cell[word_addr][8*addr_i[1:0]+:8] <= write_data_i[7:0];
        rv32_inst_pkg::MEM_WIDTH_HALF: mem_cell[word_addr][16*addr_i[1:0]+:16] <= write_data_i[15:0];
        rv32_inst_pkg::MEM_WIDTH_WORD: mem_cell[word_addr] <= write_data_i;
      endcase
    end
